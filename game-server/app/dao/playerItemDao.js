...
/**
 * Get an user's item by playerId and itemId
 * @param {Number} playerId.
 * @param {Number} itemId item_id in game_master.item_data
 * @param {function} cb Callback function.
 * @returns {Object} player's item data
 */
playerItemDao.getByItemId = function(mysqlc, playerId, itemId, cb) {
	var selectSQL = 'select * from player_item where player_id=? and item_id=?';
	var args = [playerId, itemId];
...
playerItemDao.add = function(mysqlc, id, playerId, itemId, num, cb) {
  var insertSQL = 
    'insert into player_item(id, player_id, item_id, num, created_on, updated_on) values (?,?,?,?, ?,?)';
...
playerItemDao.update = function(mysqlc, id, num, cb) {
  var updateSQL = 
    'update player_item set num=num+?, updated_on=? where id=?';
