...
var battlerPulse = function(playerForceCard, sortIndex) {
  if (playerForceCard.pulse) { // pet_skill_effect, like zhongdu/lengdong
...
async.auto({
  npc: function(callback) {
    npcUnitAllData.get(connection, npcId, function(err, res) {
      if (!!err || !res) {
        console.log('Get npc data failed!');
        console.log(err);
      }
      callback(err, res);
    });
  ...
  player: function(callback) {
    playerUnitAllData.get(connection, playerId, function(err, res) {
      if (!!err || !res) {
        console.log('Get npc data failed!');
        console.log(err);
      }
  ...
  if (!!err || !results) {
    ...
    //console.log(results);
    var npcForce = unitForce(results.npc);
    var playerForce = unitForce(results.player);
    console.log(npcForce);
    console.log(playerForce);
    var prepare = battlePrepare(results.npc, npcForce, results.player, playerForce);

    var battleRes = {};
    var firstForce;
    var secondForce;
    if (npcForce.unitAGI >= playerForce.unitAGI) {
      firstForce  = battlerInit(npcForce, 1);     // _group for p1
      secondForce = battlerInit(playerForce, 2);  // _group for p2
    }
    else {
      firstForce  = battlerInit(playerForce, 1);
      secondForce = battlerInit(npcForce, 2);
    }
